...
CSV = Path(r"etl-bench/data/raw/nyc_taxi/yellow/2016/03/yellow_tripdata_2016-03.csv").resolve()
OUT = Path("etl-bench/data/silver/nyc_taxi_parquet/daily"); OUT.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    con.execute(f"""
        ...
        TO '{OUT.as_posix()}'
        (FORMAT PARQUET, PARTITION_BY (y,m), COMPRESSION 'SNAPPY');
    """)
    ...
    out_dir = Path(f"etl-bench/results/raw_runs/{run_id}"); out_dir.mkdir(parents=True, exist_ok=True)
